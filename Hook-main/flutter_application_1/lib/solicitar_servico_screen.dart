...
class _ResumoServicoScreenState extends State<ResumoServicoScreen> {
  ...
  Future<void> _confirmarSolicitacao() async {
    ...
    try {
      // Envia os dados para a API sem travar serviços que não são guincho
      ...
      final idRaw = solicitacao['id'] ??
          (solicitacao['solicitacao'] as Map?)?['id'];

      final solicitacaoId = idRaw is int
          ? idRaw
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    _formasPagamento[_formaPagamento]!,
                                    style: const TextStyle(
                                      fontSize: 12,
                                      color: cinzaTexto,
                                    ),
                      ...
                      child: Padding(
                        ...
                        child: SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed:
                                _enviando ? null : _confirmarSolicitacao,
                            style: ElevatedButton.styleFrom(
                              backgroundColor: pretoPrincipal,
                              foregroundColor: Colors.white,
                              disabledBackgroundColor:
                                  pretoPrincipal.withOpacity(0.6),
...
class _ProcurandoProfissionalScreenState
  ...
  static const Duration intervaloAtualizacao = Duration(seconds: 1);
  ...
  Future<void> _consultarStatus() async {
    ...
    try {
      final resp = await ApiService.instance.consultarSolicitacao(
      ...
      final novaMensagem = (resp['mensagem'] as String?) ?? _mensagem;
      ...
      if (motorista != null) {
        ...
        final latitude = motorista['latitude'];
        final longitude = motorista['longitude'];
        ...
        if (latitude != null && longitude != null) {
          ...
          if (lat != null && lng != null) {
            await _atualizarPosicaoMotorista(LatLng(lat, lng));
          }
        }
      }
  ...
  Future<void> _atualizarPosicaoMotorista(LatLng novaPosicao) async {
    if (!mounted) return;

    final posicaoAnterior = _ultimaPosicaoMotorista;
    _ultimaPosicaoMotorista = novaPosicao;

    setState(() {
      _markers = {
        _markers.firstWhere(
          (marker) => marker.markerId.value == 'encontro',
        ),
        Marker(
          ...
          infoWindow: InfoWindow(
            title: _motorista?['nome'] as String? ?? 'Motorista',
          ),
        ),
      };
    });
    ...
    if (posicaoAnterior == null ||
        _distanciaAproximada(posicaoAnterior, novaPosicao) > 5) {
      await _buscarRota(novaPosicao, widget.coordenada);
    }
  }

  double _distanciaAproximada(LatLng a, LatLng b) {
    const fatorLatitude = 111000.0;
    final diferencaLatitude = (a.latitude - b.latitude).abs();
    final diferencaLongitude = (a.longitude - b.longitude).abs();

    return (diferencaLatitude * fatorLatitude) +
        (diferencaLongitude * fatorLatitude * 0.85);
  }

  void _abrirPainelMotorista() {
    if (!_sheetController.isAttached) return;

    if (_sheetController.size < _tamanhoAceitePainel - 0.02) {
